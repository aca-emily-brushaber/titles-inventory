...
function SidebarProvider({
  ...
  // Restore persisted sidebar state from cookie after mount to avoid SSR mismatch.
  React.useEffect(() => {
    ...
    if (match) {
      ...
      if (persisted !== defaultOpen) _setOpen(persisted)
    }
...
let _skelCounter = 0
const SKEL_WIDTHS = [72, 58, 85, 64, 50, 78, 55, 68, 82, 60]
...
function SidebarMenuSkeleton({
  ...
  const width = React.useMemo(() => {
    return `${SKEL_WIDTHS[_skelCounter++ % SKEL_WIDTHS.length]}%`
  }, [])
