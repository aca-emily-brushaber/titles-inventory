...
export function TitleDetailClient({
  ...
  const [titleGroup] = useState<AssignmentGroup>(title.assignment_group)
    ...
    <header className="bg-background/60 backdrop-blur-md border-b border-border px-4 lg:px-8 pt-4 pb-3 shrink-0">
      <div className="flex items-start gap-3 max-w-[1600px] mx-auto">
        <Button variant="ghost" size="icon" className="size-8 mt-0.5 shrink-0" asChild>
          <Link href={`/queue?queue=${titleGroup}`} aria-label="Back to queue">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1.5">
            <h1 className="text-base font-bold text-foreground tracking-tight">
              {ASSIGNMENT_GROUP_LABELS[titleGroup]}{" "}
              <span className="font-mono text-muted-foreground">#{title.id.slice(0, 8)}</span>
            </h1>
            <Badge
              className={cn(
            ...
            <Badge variant="outline" className="text-[10px] font-semibold px-2 py-0 max-w-[min(100%,280px)] truncate" title={title.assignment_status}>
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
            ...
            <MetaChip
              ...
              label="Due"
              value={new Date(title.due_date).toLocaleDateString("en-US", {
                month: "short",
                day: "numeric",
                year: "numeric",
              })}
            ...
            <MetaChip
              icon={IconClock}
              label="Client age (days)"
              value={title.client_age != null ? String(title.client_age) : "—"}
            />
            <Separator orientation="vertical" className="h-3.5 hidden sm:block" />
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
      ...
      <div className="lg:hidden px-4 py-2 border-b border-border shrink-0">
        <TitleWorkflowRail variant="mobile" checked={workflowChecked} onCheckedChange={setWorkflowChecked} />
      </div>

      <div className="flex flex-1 min-h-0">
        <div className="hidden lg:flex shrink-0 w-[296px] border-r border-border py-4 pl-4 pr-2 overflow-hidden">
          <TitleWorkflowRail variant="desktop" checked={workflowChecked} onCheckedChange={setWorkflowChecked} />
        </div>
        ...
        <div id="title-scroll-panel" className="flex-1 min-w-0 overflow-y-auto">
          <div className="flex flex-col gap-6 px-4 lg:px-8 py-6 max-w-[1200px]">
            ...
            <div id="title-timeline">
              <TitleHistoryPanel
                titleOpenedAt={title.created_at}
                transfers={transfers}
                currentGroup={titleGroup}
              />
            </div>

            <div id="title-fields">
              <TitleFieldGrid title={title} />
            </div>
